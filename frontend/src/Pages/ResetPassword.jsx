...
const ResetPassword = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Axios throws on non-2xx, so if we’re here, it’s successful
      setSuccess(true);
      setTimeout(() => navigate("/"), 5000);
    } catch (error) {
      // Handle both network & API errors
      const message =
        error.response?.data?.errors || "Failed to reset password.";
      setError(message);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-black">
      <div className="bg-gray-950 bg-opacity-90 rounded-2xl shadow-2xl p-8 w-full max-w-md">
        <h1 className="text-3xl font-extrabold text-white mb-2 text-center tracking-wide">
          Reset Password
        </h1>
